        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    cv2.imwrite('Training_Compressed_LR_Image_Compute_Bpp.jpg', lr_img, [int(cv2.IMWRITE_JPEG_QUALITY),quality])

                    # get lr image size
                    save_img_path='Training_Compressed_LR_Image_Compute_Bpp.jpg'
                    ...
                    lrgt_img_y = bgr2ycbcr(gtl_img, only_y=True)#gtl_img #lrgt_img
                    psnr_y_lr = util.calculate_psnr(lr_img_y * 255, lrgt_img_y * 255)
                    #ssim_y_lr = util.calculate_ssim(lr_img_y * 255, lrgt_img_y * 255)
                    avg_psnr_y_lr += util.calculate_psnr(lr_img_y * 255, lrgt_img_y * 255)

                avg_psnr_y = avg_psnr_y / idx
                avg_psnr_y_lr = avg_psnr_y_lr / idx
                bpp=bpp/idx
                
                # log
                logger.info('# Validation # PSNR: {:.4e}, PSNR_LR: {:.4e}, bpp: {:.6f}.'.format(avg_psnr_y, avg_psnr_y_lr, bpp))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> PSNR_Y: {:.4e}, PSNR_Y_LR: {:.4e}, bpp: {:.6e}.'.format(
                    epoch, current_step, avg_psnr_y, avg_psnr_y_lr, bpp))
                # tensorboard logger
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
                    #####tb_logger.add_scalar('psnr', avg_psnr, bpp,  current_step)#, avg_psnr_lr
                    tb_logger.add_scalar('psnr', avg_psnr_y, avg_psnr_y_lr, bpp,  current_step)
