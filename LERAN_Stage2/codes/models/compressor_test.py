    ...
    def __call__(self, tensors, out_type=np.uint8, min_max=(0, 1)):
        ...
        for tensor in tensors:
            ...
            with BytesIO() as f:
                ...
                cv2.imwrite('Testing_Compressed_LR_Image.jpg', img, encode_param)
                
                t1_test = time.time()
                total_time1_test1=t1_test-t0_test
                print("===> Average JPEG Encoding Time: %.4f sec." % (total_time1_test1)) #(t1 - t0))
                
                t0_test = time.time()
                
                img_jpg=cv2.imread('Testing_Compressed_LR_Image.jpg')
            ...
            print("===> Average JPEG Decoding Time: %.4f sec." % (total_time1_test2)) #(t1 - t0))
            
        return torch.stack(results, 0).to(torch.device('cuda')), total_time1_test1, total_time1_test2