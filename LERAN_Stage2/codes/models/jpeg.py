...
class DiffJPEG(nn.Module):    
    ...
    def forward(self, x):
        ...
        t1_test = time.time()
        t1_test = time.time()
        total_time1_test=t1_test-t0_test
        ###print("===> Average JPEG Encoding Time: %.4f sec." % (total_time1_test)) #(t1 - t0))

        t0_test = time.time()
        recovered = self.decompress(y, cb, cr, org_height, org_width)
        
        total_time1_test=t1_test-t0_test
        ###print("===> Average JPEG Decoding Time: %.4f sec." % (total_time1_test)) #(t1 - t0))
