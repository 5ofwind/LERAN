...
class LERAN_Stage2(BaseModel):
    def __init__(self, opt):
        super(LERAN_Stage2, self).__init__(opt)
        if opt['dist']:
    ...
    def optimize_parameters(self, step):
        ...
        y_ = LR_
        x_samples=self.netG2(x=y_,network_comp_quality=self.current_train_quality) ###, rev=True)
    ...
    def upscale(self, LR_img, scale, gaussian_scale=1):
        ...
        with torch.no_grad():
            ...
            t1 = time.time()
            total_time1=t1-t0
            print("===> Average Upsampling Time: %.4f sec." % (total_time1))
